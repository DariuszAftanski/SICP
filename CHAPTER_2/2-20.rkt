...
deﬁne such procedures is to use define with dotted-tail notation. In a procedure
...
value will be a list of any remaining arguments. For instance, given the
definition:
...
If we evaluate (f 1 2 3 4 5 6) then in the body of f, x will be 1, y will be 2,
...
If we evaluate (g 1 2 3 4 5 6) then in the body of g, w will be the
list (1 2 3 4 5 6).
|#

(
  define same-parity (lambda va_args
    (
      if (or (null? (cdr va_args))
             (null? (cdr (cdr va_args)))
         )
         (append (list (car va_args)))
         (append (list (car va_args)) (apply same-parity (cdr (cdr va_args))))
    )
  )
)

; Test cases:
(display "Evaluate: (same-parity 1 2 3 4 5 6 7) = ")
